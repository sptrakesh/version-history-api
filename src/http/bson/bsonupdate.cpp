...
void spt::http::bson::handleUpdate( const nghttp2::asio_http2::server::request& req,
    ...
    req.on_data([context, st, &req, &res](const uint8_t* chars, std::size_t size)
    {
      ...
      const auto vfn = [&]()
      {
        ...
        if ( req.method() == "PUT" )
        {
          ...
          if ( std::string_view{ oid->to_string() } != parts[4] )
          {
            LOG_WARN << "Path id different from object id in payload";
            return false;
          }
        }
...
void spt::http::bson::handleReplace( const nghttp2::asio_http2::server::request& req,
    const nghttp2::asio_http2::server::response& res )
{
  const auto st = std::chrono::steady_clock::now();
